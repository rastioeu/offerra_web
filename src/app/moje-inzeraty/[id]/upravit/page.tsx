...
export default async function ListingEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) redirect(`/login?next=/moje-inzeraty/${id}/upravit`);
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-4 py-8 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between gap-4">
        <Link href="/moje-inzeraty" className="text-sm text-link hover:underline">
          ← Moje inzeráty
        </Link>
      ...
      <ListingEditorForm property={property} />
    </main>
  );
}
