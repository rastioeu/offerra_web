 ...
 * (`bg-on-photo-surface`, vždy pilulka aj v poslednej hodine — nemizne
 * ako mimo fotky), farebná logika sa nesie len v texte: akcentová
 * oranžová bežne, `danger` v poslednej hodine, tlmená pri expirovanej
 * ponuke. Rovnaká pozícia, rovnaký typ pozadia, jeden dizajn systém —
 * líši sa len farba textu podľa stavu.
 *
 * IKONA HODÍN (Rastio, 17.9.2026, druhé kolo: „keď sa oba badge
 * zobrazujú naraz, malá ikonka pred textom nech je jasné, ktorý je
 * ktorý") — TÁ ISTÁ ikona ako v ne-`onPhoto` variante nižšie, len
 * menšia — platnosť KONKRÉTNEJ ponuky, na rozdiel od kalendára na
 * `DeadlineBadge` (termín INZERÁTU).
 */
export function OfferCountdownPill({
  ...
  if (onPhoto) {
    ...
    return (
      <span className={`inline-flex w-fit items-center gap-1 rounded-full bg-on-photo-surface px-2.5 py-[3px] text-xs font-semibold ${color}`}>
